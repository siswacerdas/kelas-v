/**
 * app-jurnal.js — logika halaman input-jurnal.html
 * Mirror dari app.js/app-kognitif.js, memakai MPLS_JURNAL_CATEGORIES dan
 * endpoint backend yang TERPISAH (namaJurnal / allJurnal / type: "jurnal")
 * supaya tidak bentrok/regresi dengan data MPLS non-kognitif & kognitif yang
 * sudah berjalan.
 * Bergantung pada mpls-data.js, mpls-jurnal-data.js, dan config.js (dimuat sebelum file ini).
 */
...
/* ── ACCESS GATE (sama seperti modul non-kognitif/kognitif) ──────────── */
function initGate() {
...
function initCategories() {
  ...
  MPLS_JURNAL_CATEGORIES.forEach((cat, idx) => {
...
function totalScaleFields() {
  return MPLS_JURNAL_CATEGORIES.reduce((s, c) => s + c.items.length, 0);
}
function updateProgress() {
  const total = totalScaleFields();
  let filled = 0;
  MPLS_JURNAL_CATEGORIES.forEach((cat) =>
...
function updateCategoryCounts() {
  document.querySelectorAll(".cat-card").forEach((card) => {
    const cat = MPLS_JURNAL_CATEGORIES.find((c) => c.key === card.dataset.key);
...
function resetForm() {
  ...
  document.getElementById("inp-cuplikan").value = "";
  updateProgress();
  updateCategoryCounts();
}
...
function applyExistingData(data) {
  ...
  document.getElementById("inp-cuplikan").value = data["Cuplikan Tulisan Siswa"] || "";
  updateProgress();
  updateCategoryCounts();
}
...
async function onStudentChange() {
  ...
  try {
    ...
    if (json && json.found) {
      ...
      setSaveStatus("Siswa baru — belum ada data asesmen menulis tersimpan.");
    }
  } catch (err) {
...
async function saveData() {
  ...
  const payload = Object.assign({}, state.data, {
    ...
    "Cuplikan Tulisan Siswa": document.getElementById("inp-cuplikan").value || "",
