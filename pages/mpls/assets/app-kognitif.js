/**
 * app-kognitif.js — logika halaman input-kognitif.html
 * Mirror dari app.js (non-kognitif), memakai MPLS_KOGNITIF_CATEGORIES dan
 * endpoint backend yang TERPISAH (namaKognitif / allKognitif / type: "mpls_kognitif")
 * supaya tidak bentrok/regresi dengan data MPLS non-kognitif yang sudah berjalan.
 * Bergantung pada mpls-data.js, mpls-kognitif-data.js, dan config.js (dimuat sebelum file ini).
 */
...
/* ── ACCESS GATE (sama seperti modul non-kognitif) ──────────────── */
function initGate() {
...
function initCategories() {
  ...
  MPLS_KOGNITIF_CATEGORIES.forEach((cat, idx) => {
...
function totalScaleFields() {
  return MPLS_KOGNITIF_CATEGORIES.reduce((s, c) => s + c.items.length, 0);
}
function updateProgress() {
  const total = totalScaleFields();
  let filled = 0;
  MPLS_KOGNITIF_CATEGORIES.forEach((cat) =>
...
function updateCategoryCounts() {
  document.querySelectorAll(".cat-card").forEach((card) => {
    const cat = MPLS_KOGNITIF_CATEGORIES.find((c) => c.key === card.dataset.key);
...
async function onStudentChange() {
  ...
  try {
    ...
    if (json && json.found) {
      ...
      setSaveStatus("Siswa baru — belum ada data asesmen kognitif tersimpan.");
    }
  } catch (err) {
